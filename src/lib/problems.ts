...
for(const c of cases){
  try{
    ...
    console.log(ok?'✓ PASS':'✗ FAIL','| Input: nums=['+c.n+'] target='+c.t+' | Expected: ['+c.e+'] | Got:',(r?'['+r+']':r));
    if(ok)p++;
  }catch(e){console.log('✗ ERROR | Input: ['+c.n+'] target='+c.t+' |',e.message);}
}
...
print(f'\\n{p}/{len(cases)} test cases passed.')`,
  ...
  // ── 3. Climbing Stairs ───────────────────────────────────────────────────────
  {
    ...
    jsRunner: jsIIFE(`
const cases=[{n:1,e:1},{n:2,e:2},{n:3,e:3},{n:5,e:8},{n:10,e:89},{n:20,e:10946}];
let p=0;
for(const c of cases){
  try{
...
console.log('\\n'+p+'/'+cases.length+' test cases passed.');`),
    pyRunner: `
sol=Solution()
cases=[{'n':1,'e':1},{'n':2,'e':2},{'n':3,'e':3},{'n':5,'e':8},{'n':10,'e':89},{'n':20,'e':10946}]
p=0
for c in cases:
    try:
...
for(const c of cases){
  try{
    const r=maxProfit([...c.p]);
    const ok=r===c.e;
    console.log(ok?'✓ PASS':'✗ FAIL','| prices='+JSON.stringify(c.p)+'| Expected:'+c.e+'| Got:'+r);
    if(ok)p++;
  }catch(e){console.log('✗ ERROR |',e.message);}
}
console.log('\\n'+p+'/'+cases.length+' test cases passed.');`),
...
print(f'\\n{p}/{len(cases)} test cases passed.')`,
  ...
  // ── 14. Binary Search ────────────────────────────────────────────────────────
  {
    ...
    jsRunner: jsIIFE(`
const cases=[
  {n:[-1,0,3,5,9,12],t:9,e:4},
  {n:[-1,0,3,5,9,12],t:2,e:-1},
  {n:[5],t:5,e:0},
  {n:[5],t:3,e:-1},
  {n:[1,3,5,7,9],t:1,e:0},
  {n:[1,3,5,7,9],t:9,e:4},
...
for(const c of cases){
  try{
    const r=numIslands(c.g.map(row=>[...row]));
    const ok=r===c.e;
    console.log(ok?'✓ PASS':'✗ FAIL','| Expected:'+c.e+'| Got:'+r);
    if(ok)p++;
  }catch(e){console.log('✗ ERROR |',e.message);}
...
for c in cases:
    try:
        r=sol.numIslands([row[:] for row in c['g']])
        ok=r==c['e']
        print('✓ PASS' if ok else '✗ FAIL','| Expected:'+str(c['e'])+'| Got:'+str(r))
        if ok: p+=1
    except Exception as ex:
...
for(const c of cases){
  try{
    const r=subsets([...c.n]);
    const ok=norm(r)===norm(c.e)&&r.length===c.e.length;
    console.log(ok?'✓ PASS':'✗ FAIL','| Expected '+c.e.length+' subsets | Got '+r.length);
    if(ok)p++;
  }catch(e){console.log('✗ ERROR |',e.message);}
}
console.log('\\n'+p+'/'+cases.length+' test cases passed.');`),
...
for c in cases:
    try:
        r=sol.subsets(c['n'][:])
        ok=norm(r)==norm(c['e']) and len(r)==len(c['e'])
        print('✓ PASS' if ok else '✗ FAIL','| Expected',len(c['e']),'subsets | Got',len(r))
        if ok: p+=1
    except Exception as ex:
        print('✗ ERROR |',str(ex))
print(f'\\n{p}/{len(cases)} test cases passed.')`,
...
for c in cases:
    try:
        ...
        ok=is_palin(r) and c['s'] in (c['s']) and (r==c.get('e',r) if 'e' in c else len(r)>=c.get('min_len',1))
...
for c in cases:
    try:
        ...
        ok=r==c['e']
        print('✓ PASS' if ok else '✗ FAIL','| Expected:'+repr(c['e'])+'| Got:'+repr(r))
        if ok: p+=1
    except Exception as ex:
        print('✗ ERROR |',str(ex))
print(f'\\n{p}/{len(cases)} test cases passed.')`,
...
print(f'\\n{p}/{len(cases)} test cases passed.')`,
  ...
  // ── 30. N-Queens ─────────────────────────────────────────────────────────────
  {
    ...
    jsRunner: jsIIFE(`
const expected={1:1,4:2,5:10,6:4,8:92};
const cases=[1,4,5,6];
let p=0;
for(const n of cases){
  try{
    const r=solveNQueens(n);
    const ok=r.length===expected[n];
    console.log(ok?'✓ PASS':'✗ FAIL','| n='+n+'| Expected '+expected[n]+' solutions | Got '+r.length);
    if(ok)p++;
...
console.log('\\n'+p+'/'+cases.length+' test cases passed.');`),
    pyRunner: `
sol=Solution()
expected={1:1,4:2,5:10,6:4,8:92}
cases=[1,4,5,6]
p=0
for n in cases:
    try:
        ...
        ok=len(r)==expected[n]
        print('✓ PASS' if ok else '✗ FAIL','| n='+str(n)+'| Expected '+str(expected[n])+' solutions | Got '+str(len(r)))
        if ok: p+=1
    except Exception as ex:
